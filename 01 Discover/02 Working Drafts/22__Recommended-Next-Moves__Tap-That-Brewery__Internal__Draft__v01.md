...
## Move 0 – Close the three data blockers (this fortnight, before Design begins)

No Design-phase asset should be built on unreconciled inputs. Three gates, all already named in D06:

| Blocker | Why it blocks | Owner | Timing |
|---|---|---|---|
| **Full census data + business docs** to Jules (Google Drive) – carried action from the discovery session | Baselines for churn, spend, delivery pricing; all TBC targets in #19 lock against it | **Clent** | This week |
...
**What:** the campaign layer the team has known it needed – young families first (~45%, connection-led, the backyard as the venue), then tradies (kudos, the shed build-out) and high-volume drinkers (the one segment where the savings maths leads). Built on the 90-day nurture matched to the 3-month buying cycle, aimed at the next giveaway close (H3: lift 22–25 conversions per cycle to 35+). Includes the partner buy-in track (the non-beer third of the tap wall answering "my wife won’t let me") and the unmarketed health angle (gluten-reduced, lower-carb) as a test cell (H7).
...
*Feeds into: D07 Discovery Readout, D08 Gate 1 Deck, and the 02 Design phase – DS03 Marketing Plan (which turns this sequence into the full plan), DS01 Customer Profile, DS04 Brand Strategy.*

...
## The customer referral program – promoting what already exists

The mechanics exist and are photo-verified on one in-venue sign: **friend buys a keg system = 1,000 Tap Tokens; venue/sports club/restaurant referral = 1,000; friend gets a refill = 500.** The problem is nobody hears about it. Promotion plan:
...
- Tap Token dollar value and redemption rules unconfirmed – needed to sanity-check reward generosity before scaling promotion (1,000 tokens must feel worth a system referral without breaking margin).
- Benchy terms (pricing, margin, Kegland supply) and the cousin’s commercial expectations unconfirmed – settle before the showroom demo ships.
- Club white-label assumes brewing capacity for custom decals/runs at current scale – confirm with Chris; 10%/10% maths to be modelled against per-keg margin (needs P&L data).
- Never Quit perk redemptions currently unmeasured; instrument via GoTab discount codes before judging the partnership.
- Smoked Barbecue Co collab results (redemptions, sales) from the first Father’s Day run not yet in hand – request before repeating.
- Partner names beyond those observed (contacts.md) are categories, not booked conversations.

---

*Feeds into: #43 In-Market Activation Plan, #45 Campaign Calendar, wholesale one-pager and referral collateral builds, Deploy-phase pipeline tracker.*
